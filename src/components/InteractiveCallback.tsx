...
export default function InteractiveCallback() {
  ...
  const [project, setProject] = useState('Vinhomes Smart City');
  const [apartmentType, setApartmentType] = useState('2PN');
  
  // Helper to determine accurate default prices (in millions VND) for each project & apartment type
  const getInitialPriceForProjectAndType = (proj: string, aptType: string): number => {
    if (proj === 'Vinhomes Smart City') {
      switch (aptType) {
        case 'Studio': return 2600; // 2.6 Tỷ
        case '1PN': return 3400;    // 3.4 Tỷ
        case '2PN': return 4800;    // 4.8 Tỷ
        case '3PN': return 6500;    // 6.5 Tỷ
        default: return 4800;
      }
    } else if (proj === 'Imperia Sky Park') {
      switch (aptType) {
        case 'Studio': return 2800; // 2.8 Tỷ
        case '1PN': return 3500;    // 3.5 Tỷ
        case '2PN': return 5000;    // 5.0 Tỷ
      ...
      switch (aptType) {
        ...
        case '1PN': return 4800;    // 4.8 Tỷ
        case '2PN': return 6800;    // 6.8 Tỷ
        case '3PN': return 9500;    // 9.5 Tỷ
        default: return 6800;
      }
    }
  };

  const [budget, setBudget] = useState(4800); // triệu VND (Mặc định 4.8 tỷ cho 2PN Vinhomes Smart City)
  const [downPaymentPct, setDownPaymentPct] = useState(20); // % vốn tự có
  const [loanTerm, setLoanTerm] = useState(35); // năm, mặc định tối đa 35 năm (420 tháng)
  ...
  // Assumed interest rate after incentive period: 10% per year
  const annualInterestRate = 0.10;
  const monthlyInterestRate = annualInterestRate / 12;
  const totalMonths = loanTerm * 12; // loanTerm in years
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!clientName.trim() || !clientPhone.trim()) {
      setFormError('Vui lòng nhập đầy đủ Tên và Số điện thoại để Hoàng Yến hỗ trợ.');
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        setSendFeedback({
          type: 'success',
          message: 'Hệ thống đã tự động gửi email thông báo thành công đến Hoàng Yến!'
        });
      } else {
  ...
  const getZaloPrefilledUrl = () => {
    const textMessage = `Xin chào Hoàng Yến! Tôi là ${clientName} (${clientPhone}). Tôi vừa làm tính toán dòng tiền trên hoangyenbds.vn:
- Dự án: ${project}
- Căn hộ mục tiêu: ${apartmentType}
- Giá trị căn: ${budget / 1000} Tỷ VNĐ
- Vốn tự có: ${equityNeeded / 1000} Tỷ (${downPaymentPct}%)
- Vay ngân hàng: ${loanAmount / 1000} Tỷ trong ${loanTerm} năm (${totalMonths} tháng) với lãi suất khoảng 10%/năm.
      ...
      <div className="glass-card-red rounded-3xl p-6 sm:p-8 relative border border-white/10 overflow-hidden">
        ...
        <div className="flex flex-col items-start mb-6">
          <div className="flex items-center gap-2 px-3 py-1 bg-brand-red/20 border border-brand-red/30 rounded-full text-brand-gold text-xs font-mono font-semibold uppercase tracking-wider mb-3">
            ...
            Bảng Tính Ước Tính Dòng Tiền Dự Án
          </div>
        ...
        {!isSubmitted ? (
                ...
                >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {['Vinhomes Smart City', 'Imperia Sky Park', 'Masteri Grand Coast', 'Masterise Hà Nội Sesson Gaden'].map((p) => (
                        <button
                          ...
                          onClick={() => {
                            setProject(p);
                            setBudget(getInitialPriceForProjectAndType(p, apartmentType));
                          }}
                          className={`py-3 px-4 rounded-xl text-xs font-semibold border transition-all text-center ${
                            project === p
                              ? 'bg-brand-red/10 border-brand-red text-white shadow-[0_0_15px_rgba(215,25,32,0.15)] font-bold'
                      ...
                      {['Studio', '1PN', '2PN', '3PN'].map((apt) => (
                        <button
                          ...
                          onClick={() => {
                            ...
                            setBudget(getInitialPriceForProjectAndType(project, apt));
                          }}
                  ...
                  <div className="bg-white/5 rounded-2xl p-4 border border-white/5">
                    <p className="text-xs text-gray-400 leading-relaxed font-sans">
                      💡 <strong className="text-brand-gold">Tư vấn từ Hoàng Yến:</strong> Các dự án này đều sở hữu liên kết vùng đắc địa, quy hoạch xanh mát và bàn giao nội thất sang trọng, cam kết thanh khoản và hiệu suất cho thuê hàng đầu thủ đô.
                    </p>
                  </div>
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-1">
                        <label className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider">
                          Thời gian vay ({loanTerm} năm / {loanTerm * 12} tháng)
                        </label>
                      </div>
                      <select
                        value={loanTerm}
                      ...
                      >
                        ...
                        <option value={35}>35 Năm (Tối đa 420 tháng)</option>
                      </select>
                    </div>
                  </div>
                  ...
                  <div className="bg-premium-dark/60 rounded-2xl p-4 border border-white/5 space-y-2 text-xs">
                    ...
                    <div className="flex justify-between items-center text-gray-400">
                      <span className="flex items-center gap-1">
                        ...
                        <small className="text-[9px] text-gray-500">(Gốc + Lãi khoảng 10%/năm)</small>
                      </span>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                      ...
                      <input
                        type="tel"
                        required
                        placeholder="Ví dụ: 0865 xxx xxx"
                        value={clientPhone}
                        onChange={(e) => {
                    ...
                    <div className="p-3 bg-brand-gold/10 border border-brand-gold/25 rounded-2xl">
                      <p className="text-[11px] text-brand-gold/90 leading-relaxed">
                        💎 Hoàng Yến sẽ liên hệ phân tích chuyên sâu cho anh/chị chính sách hỗ trợ lãi suất 0% lên tới 24-36 tháng áp dụng cho từng phân khu và quỹ căn hộ phù hợp!
                      </p>
                    </div>
          ...
          >
            ...
            <p className="text-xs text-brand-gold font-semibold font-sans">
              🎁 Nhấn nút bên dưới để chuyển trực tiếp bảng tính toán chi tiết này qua Zalo cho Hoàng Yến!
            </p>
